...
// --- SOUND SETUP (Assumes setup is done once per project) ---
...
const Multiplication = ({ navigation }) => {
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < TOTAL_QUESTIONS; i++) {
      // Difficulty scales for multiplication (focus on times tables 2-12)
      const num1 = Math.floor(Math.random() * 11) + 2; // Numbers from 2 to 12
      const num2 = Math.floor(Math.random() * (5 + i)) + 2; // Second number difficulty increases
      const correctAnswer = num1 * num2;

      let options = new Set([correctAnswer]);
      while (options.size < OPTIONS_COUNT) {
        // Generate plausible wrong answers
        const randomFactor = Math.random() < 0.5 ? num1 : num2;
        const offset = Math.floor(Math.random() * 3) - 1; // -1, 0, or 1
        if (offset !== 0) {
            const wrongAnswer = correctAnswer + (offset * randomFactor);
            if (wrongAnswer > 0) {
              options.add(wrongAnswer);
            }
        } else { // add a completely random number if offset is 0
            options.add(Math.floor(Math.random() * 20) + correctAnswer - 10)
        }
      }
      newQuestions.push({
        question: `${num1} × ${num2}`, // Using multiplication symbol
  ...
  // --- UI COMPONENTS ---
  // Note: These are almost identical to AdditionScreen, just theming and text changes
  const renderHeader = () => (
    ...
    <View style={styles.optionsGrid}>
      {questions[currentQuestionIndex]?.options.map((option, index) => {
        ...
        const getBackgroundColor = () => {
          if (isSelected) {
            return isCorrect ? '#4ECDC4' : '#FF6B6B'; // Universal correct/incorrect colors
          }
          if (selectedAnswer && isTheCorrectAnswer) {
            return '#4ECDC4'; // Highlight correct answer if user was wrong
          }
...
// --- STYLESHEET (Themed for Multiplication) ---
const styles = StyleSheet.create({
  ...
  progressBar: {
    height: '100%',
    backgroundColor: '#45B7D1', // THEME COLOR
    borderRadius: hp('1%'),
  },
  questionContainer: {
    flex: 1,
