...
const TIMER_DURATION = 5; // 5 seconds per question
...
const SpeedMath = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    // Reset timer and animations for the new question
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < TOTAL_QUESTIONS; i++) {
      ...
      switch (operator) {
        case '+':
          num1 = Math.floor(Math.random() * (20 + i)) + 1;
          num2 = Math.floor(Math.random() * (20 + i)) + 1;
          correctAnswer = num1 + num2;
          questionText = `${num1} + ${num2}`;
          break;
        case '-':
          const a = Math.floor(Math.random() * (25 + i)) + 5;
          const b = Math.floor(Math.random() * (25 + i)) + 1;
          num1 = Math.max(a, b);
          num2 = Math.min(a, b);
          correctAnswer = num1 - num2;
          questionText = `${num1} - ${num2}`;
  ...
  const handleTimeOut = () => {
    ...
    setIsCorrect(false);
    setSelectedAnswer('timeout'); // Special value to indicate timeout
    moveToNextQuestion();
  };
    ...
    <View>
       <View style={styles.progressHeader}>
         ...
         <View style={styles.timerContainer}>
            <Icon name="clock-fast" size={wp('4%')} color={timeLeft <= 2 ? '#FF6B6B' : '#A9A9A9'} />
            <Text style={[styles.timerText, timeLeft <= 2 && {color: '#FF6B6B'}]}>{timeLeft}s</Text>
         </View>
       </View>
      ...
      <View style={[styles.progressBarBackground, styles.timerBarContainer]}>
        <Animated.View style={[styles.timerBar, timerProgressStyle, timeLeft <= 2 && {backgroundColor: '#FF6B6B'}]} />
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  questionText: { color: '#FFF', fontSize: wp('12%'), fontWeight: '700', textAlign: 'center' },
  optionsGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    justifyContent: 'space-between',
